...
def calculate_confidence_intervals():
    ...
    print(f"\n=== ROI Analysis (95% Confidence Interval) ===")
    ...
    if roi_lower > 0:
        ...
        print(f"   Need more bets or higher win rate to prove profitability.")
    ...
    if clv_values:
        ...
        print(f"\n=== CLV Analysis (95% Confidence Interval) ===")
        print(f"Sample Size (N): {n_clv}")
        print(f"Mean CLV: {mean_clv:.2%}")
        print(f"Standard Deviation: {std_clv:.2%}")
        ...
        if clv_lower > 0:
            ...
            print("   You are consistently beating the closing line!")
        else:
            print("⚠️  Edge not statistically proven yet.")
            print(f"   CLV is positive but needs more samples for significance.")
    else:
        print("\n⚠️  Could not calculate CLV stats (missing closing odds).")
    
    # 5. Additional Stats
    print(f"\n=== Additional Statistics ===")
    wins = bets[bets['result'] == 'WIN']
    losses = bets[bets['result'] == 'LOSS']
    win_rate = len(wins) / n_bets if n_bets > 0 else 0
    
    print(f"Win Rate: {win_rate:.2%} ({len(wins)} wins / {n_bets} bets)")
    print(f"Average Odds: {bets['placed_odds'].mean():.2f}")
    print(f"Total Wagered: ${bets['stake'].sum():.2f}")
